...
export default function MapComponent({ locations, initialCenter, isDarkMode }: MapComponentProps) {
    ...
    const defaultMarkerSVG = `url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24' fill='${encodeURIComponent(defaultMarkerColor)}'><path d='M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 0 1 18 0z'/><circle cx='12' cy='10' r='3' fill='white'/></svg>")`;
    const selectedMarkerSVG = `url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24' fill='${encodeURIComponent(selectedMarkerColor)}'><path d='M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 0 1 18 0z'/><circle cx='12' cy='10' r='3' fill='white'/></svg>")`;
    useEffect(() => {
    ...
    const drawMarkers = () => {
      ...
      <div class="flex items-center gap-3 font-sans">
        ...
        <div><h3 class="m-0 font-bold text-base text-gray-900">${loc.restaurantName}</h3></div>
            ...
            el.addEventListener("mouseenter", () => (iconEl.style.transform = "scale(1.2)"));
            el.addEventListener("mouseleave", () => (iconEl.style.transform = "scale(1)"));
