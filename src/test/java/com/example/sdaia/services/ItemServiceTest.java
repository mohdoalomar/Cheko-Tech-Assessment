...
public class ItemServiceTest {
    ...
    void getAllItems_ShouldReturnItemPageResponseDTO() {
        ...
        when(itemRepository.findAll(any(Specification.class), any(Pageable.class))).thenReturn(itemPage);
        when(itemRepository.count(any(Specification.class))).thenReturn(2L);

        //execute
        ItemPageResponseDTO result = itemService.getAllItems(
                ItemCategory.BREAKFAST, "price", true, true, "test", 10, 0);
    ...
    void countItemsByCategory_ShouldReturnMapOfCounts() {
        ...
        doAnswer(invocation -> {
            ...
            return 2L;
        }).when(itemRepository).count(any(Specification.class));
